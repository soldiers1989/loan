...
public class Splash3Activity extends BaseActivity {
		...
		builder.setPositiveButton(getResources().getString(R.string.update),
				new OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						if (mWitch == 1) {
							if (isAvilible(Splash3Activity.this, "com.android.vending")) {
	...
	private void jumpNextPage() {
		// 判断之前有没有显示过新手引导页
		ConnectionDetector cd = new ConnectionDetector(Splash3Activity.this);
		SharedPreferences sp = getSharedPreferences("config", MODE_PRIVATE);
		boolean userGuide = sp.getBoolean("is_user_guide_showed", false);
		String userLogin = sp.getString("phone", "");

		if (!userGuide) {
			startActivity(new Intent(Splash3Activity.this, Guide3Acytivity.class));
			finish();
			return;
		}
		if (cd.isConnectingToInternet2()) {
			if (userLogin == "") {
				startActivity(new Intent(Splash3Activity.this,
			...
			startActivity(new Intent(Splash3Activity.this, Login3Activity.class));
			finish();
		}
		finish();
	}
		...
		if (Environment.getExternalStorageState().equals(
			...
			utils.download(mDownloadUrl, target, new RequestCallBack<File>() {
				...
				public void onFailure(HttpException arg0, String arg1) {
					ToastUtil.showShort(Splash3Activity.this, getResources()
							.getString(R.string.download_failed));

				}
			});
		} else {
			ToastUtil.showShort(Splash3Activity.this,
